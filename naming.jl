...
!!! terminology "Problem definition"
    What structure, if any, is there to the networks that maximize the **convergence** of a simple naming game at given time $T$? Do succesful communication strategies emerge, and how do the networks that implement them look like?
...
!!! terminology
	...
	Advantages over conventional MCMC methods:
    - Inclusion of prior information
    - Simulate any temperature for free if only one constraint!
	- No burn-in required, good convergence measures
    - Can evaluate (derivatives of) $\log Z$, can evaluate $H$, density of states, etc.
	- General superior "mapping out of the problem" ability compared to MCMC, though presumably limited to small amount of dimensions compared to MCMC

Generate networks $A \sim \pi(A)$ from the prior and likelihood $\exp(\lambda c(A))$ and solve the $D$-dimensional problem with nested sampling *once* for $\lambda = 1$, then reweigh for any other value of $\lambda$ to vary the constraint $\langle c(A) \rangle = C$.

Set $C$ to high values for a given $T$. How do the posterior samples $A \sim P(A|\lambda)$ look like? Are there any favored structures?

!!! warning
    This is a **noisy likelihood** problem, because at a given point $A$ the value of $c(A)$ depends on some extra random numbers not accessible to the nested sampler. Instead, we have a distribution $p(c|A)$ from which we sample only value. We can also evaluate a Monte Carlo estimate of the mean of this distribution to get more smooth constraints or let the nested sampler control the decisions in the ABM by augmenting the problem dimensionality $D$ with extra dimensions, one for each decision made by the ABM.

## Network prior $\pi(A)$

Undirected $ER(N,p)$-network where $N$ is fixed and $O(5)$ and $p \sim Beta(.5,.5)$ to allow for a richer prior. This gives $D = N(N-1)/2 + 1$ dimensions.

## Convergence measure

Let $p_i^{(T)}$ be the normalized distribution of unique words $i = 1:N$ found in the lexicons of the $N$ agents at a given time step $T$, which depends implicitly on a given network $A$ (suppressed in the notation).
Let $u_i = p_i^{(0)} = 1/N$ be that initial distribution and $\delta_i = \delta_{ij}$ the discrete delta distribution which represents absolute convergence (agreement) on the final word $j$.
Then **define convergence** as
````math
c(T) = {KL(p^{(T)}|u) \over KL(\delta|u)} \in [0,1]
````
Here $KL(p|q)$ is the Kullback-Leibler divergence from $q$ to $p$. Concretely $c(T) = 1 - {H(p_i^{(T)}) \over \log N}$, but the above expression expresses the philosophy behind the measure, with $H(p)$ the discrete entropy of $p$.

I think $c$ is a much more interesting measure than the success rate $S$ because it will, for example, not be equal to $1$ when there are two disconnected components which have converged fully amongst themselves.
"""

# ╔═╡ fd47f9b1-ad53-4aab-a53c-a0c7e11419bc
begin
	function erdosrenyi(N, p, u)
		v = quantile(Bernoulli(p), u) .|> Int
		# Could also use TriangularIndices.jl for populating `U` from `v`
		linearindex(i, j) = (N*(N-1)÷2) - (N-i)*((N-i)-1)÷2 + j - i - 1
		U = [i < j ? v[linearindex(i, j)] : 0 for i = 1:N, j = 1:N]
		A = Hermitian(U)
	end

	"""
	Number of dimensions of a random network in our problem, i.e., the number of random numbers (RNs) needed to sample a random adjacency matrix `A` in our scheme:
	  - Sample a random Bernouilli probability from a Beta distribution (1 RN)
	  - Sample a symmetrical (0,1) matrix with zeros on the diagonal ((N*(N-1))÷2 RNs)
	"""
	ndim(N) = 1 + (N*(N-1))÷2
	...
	function prior(N, u; α = .5, β = .5)
		p = quantile(Beta(α, β), u[1])
		A = erdosrenyi(N, p, u[2:end])
	end
end

# ╔═╡ 59e1e7cb-febf-4bf9-879d-1b88d7b3adc3
"""Plot the adjacency matrix `A` as a undirected graph"""
function plotadj(A)
...
begin
	frequencies(a) = [nᵢ/length(a) for nᵢ in values(counter(a))]

	function convergence(N, inventory)
		p = frequencies(inventory)
		H = entropy(p)
		c = 1. - H/log(N)
	end
end
...
begin
	...
	function initialize(A)
		...
		properties = Dict(
			...
			:c => 0., # Convergence measure
			:graph => graph # Underlying undirected graph
		)
	...
	function agent_step!(speaker, model)
	...
	function model_step!(model)
		inventory = vcat([speaker.lexicon for speaker in allagents(model)]...)
...
begin
	"""
	Estimate the marginal likelihood by Monte Carlo integration using `M` samples of the naming game convergence played on network `A` at time step `T`
	"""
	function monte_carlo_logl(M, T, λ, A)
		...
		cs = [sampleconvergence() for _ in 1:M]
		logl = logsumexp(λ*cs) - log(M)
	end

	sampleposterior(N, chain, state) = reshape(Array(sample(chain, 1), [:parameters]), N, N)
	...
	"""
	function posterior(N, M, T, λ; α=.5, β=.5, nlive=3ndim(N))
		adj(v) = reshape(v, N, N)
		vec(A) = reshape(A, N^2)
		
		ptform(u) = vec(prior(N, u; α=α, β=β))
		logl(v) = monte_carlo_logl(M, T, λ, adj(v))
...
plotadj.([sampleposterior(𝑃₁...) for _ in 1:3])

# ╔═╡ 3536a4b6-46dc-41f4-ae98-945a2ec97986
𝑃 = let
	N = 15
	M = 200
	T = 10
...
# ╔═╡ c4b82af8-a10b-4ff5-8576-09f851ab8ab8
plotadj.([sampleposterior(𝑃...) for _ in 1:3])
