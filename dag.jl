...
@memoize function johnsontrotter(n)
	@assert n <= 10 "Exhaustive enumeration requires small n"
...
begin
	...
	function rand_Bernouilli_prob(u, α, β)
		quantile(Beta(α, β), u)
	end

	function rand_strictly_upper_triangular(u, N, p)
	...
	"""
	Number of dimensions of a random DAG in our problem, i.e., the number of random numbers (RNs) needed to sample a random DAG in our scheme:
	  - Sample a random permutation matrix (1 RN)
	  - Sample a random Bernouilli probability from a Beta distribution (1 RN)
	  - Sample a strictly upper triangular matrix ((N*(N-1))÷2 RN)
	"""
	ndim(N) = 1 + 1 + (N*(N-1))÷2
	...
	function prior(N, u; α = .5, β = .5)
		...
		U = rand_strictly_upper_triangular(u[3:end], N, p)
		A = Π' * U * Π
	end
end
